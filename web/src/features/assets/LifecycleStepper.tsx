...
export function LifecycleStepper({ state }: { state: AssetLifecycleState }) {
  ...
  if (isOffFlow) {
    return (
      <div className="flex items-center gap-3 rounded-lg border border-rose-200 bg-rose-50/70 px-4 py-3 text-sm text-rose-800">
  ...
  return (
    <div className="relative">
      {/* Background track */}
      <div className="absolute top-4 right-4 left-4 h-0.5 rounded-full bg-slate-200" />
            ...
            <li key={stage.key} className="flex flex-col items-center">
              <motion.div
                ...
                className={cn(
                  ...
                  isCurrent &&
                    'bg-white text-brand-700 ring-2 ring-brand-500 shadow-[0_0_0_4px_rgb(99_102_241_/_0.12)]',
                  !isComplete &&
                    !isCurrent &&
